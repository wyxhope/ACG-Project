...
def custom_rigid_body_simulation(config):
    ...
    waters = []
    for fluid_conf in config.get('waters', []):
        ...
        waters.append({'water': fluid, 'simulator': simulator, 'conf': fluid_conf})        
    ...
    for frame in range(num_frames):
        for curtain in curtains:
            curtain['obj'].step(dt, substeps=8000, wind_t=frame*dt)
        for net in nets:
            net['obj'].step(dt, substeps=5000, rigid_bodies=bodie)
        ...
        for body_dict in bodies:
            body_dict['obj'].apply_force(gravity * body_dict['obj'].mass, dt)
